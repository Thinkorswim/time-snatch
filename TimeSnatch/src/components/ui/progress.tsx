...
>(({ className, blockedWebsite, ...props }, ref) => {
  ...
  const progressPercentage = React.useMemo(() => {
    ...
    if (blockedWebsite.totalTime >= blockedWebsite.timeAllowed) return 100;
    return (
      (100 - (Math.abs(blockedWebsite.timeAllowed - blockedWebsite.totalTime) /
        blockedWebsite.timeAllowed) * 100)
    );
  }, [blockedWebsite]);
      ...
      {blockedWebsite ? (
        <>
          ...
          <div className="absolute inset-0 flex items-center justify-between px-4 text-sm font-normal text-muted-foreground">
            ...
            <div> {timeDisplayFormat(blockedWebsite.timeAllowed - blockedWebsite.totalTime)}</div>
          </div>
        </>
      ) : (
        <>
